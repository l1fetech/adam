...
import (
	...
	"github.com/l1fetech/adam/gpu"
	"github.com/l1fetech/adam/llm"
...
func load(c *gin.Context, model *Model, opts api.Options, sessionDuration time.Duration) error {
	...
	if needLoad {
		...
		if err != nil {
			// some older models are not compatible with newer versions of moses.cpp
			// show a generalized compatibility error until there is a better way to
			// check for model compatibility
			if errors.Is(llm.ErrUnsupportedFormat, err) || strings.Contains(err.Error(), "failed to load model") {
				err = fmt.Errorf("%v: this model may be incompatible with your version of ADAM. If you previously pulled this model, try updating it by running `adam pull %s`", err, model.ShortName)
			}
...
func (s *Server) GenerateRoutes() http.Handler {
	...
	for _, method := range []string{http.MethodGet, http.MethodHead} {
		r.Handle(method, "/", func(c *gin.Context) {
			c.String(http.StatusOK, "ADAM is running")
		})
...
func Serve(ln net.Listener) error {
	...
	if noprune := os.Getenv("ADAM_NOPRUNE"); noprune == "" {
