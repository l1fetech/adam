...
async function main() {
  ...
  await adam.setModel("neural-chat");
  adam.setSystemPrompt(systemprompt);

  // setJSONFormat is the equivalent of setting 'format: json' in the API
  adam.setJSONFormat(true);
  await adam.streamingGenerate(prompt, (word) => { process.stdout.write(word) })
}
