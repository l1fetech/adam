...
const logger = winston.createLogger({
  transports: [
    ...
    new winston.transports.File({
      filename: path.join(app.getPath('home'), '.adam', 'logs', 'server.log'),
...
function updateTray() {
  ...
  const menu = Menu.buildFromTemplate([
    ...
    { role: 'quit', label: 'Quit ADAM', accelerator: 'Command+Q' },
  ])
  ...
  tray.setToolTip(updateAvailable ? 'An update is available' : 'Adam')
...
function server() {
  const binary = app.isPackaged
    ? path.join(process.resourcesPath, 'adam')
    : path.resolve(process.cwd(), '..', 'adam')
    ...
    if (app.isPackaged) {
      if (!app.isInApplicationsFolder()) {
        const chosen = dialog.showMessageBoxSync({
          ...
          message: 'Adam works best when run from the Applications directory.',
          defaultId: 0,
          cancelId: 1,
        })
              ...
              conflictHandler: conflictType => {
                if (conflictType === 'existsAndRunning') {
                  dialog.showMessageBoxSync({
                    ...
                    detail:
                      'Another version of Adam is currently running from your Applications directory. Close it first and try again.',
                  })
                }
                return true
              },
