...
/*
#cgo CFLAGS: -I${SRCDIR}/ext_server -I${SRCDIR}/moses.cpp -I${SRCDIR}/moses.cpp/common -I${SRCDIR}/moses.cpp/examples/server
...
import (
	...
	"github.com/l1fetech/adam/api"
)

type dynExtServer struct {
	s       C.struct_dynamic_moses_server
	options api.Options
}
...
func newDynExtServer(library, model string, adapters, projectors []string, opts api.Options) (LLM, error) {
	...
	var srv C.struct_dynamic_moses_server
	...
	for i := 0; i < len(adapters); i++ {
		...
		la.scale = C.float(1.0) // TODO expose scale/weights up through ADAM UX
		la.next = nil
		if i == 0 {
	...
	if len(projectors) > 0 {
		// TODO: applying multiple projectors is not supported by the moses.cpp server yet
	...
	if debug := os.Getenv("ADAM_DEBUG"); debug != "" {
		sparams.verbose_logging = C.bool(true)
	} else {
		sparams.verbose_logging = C.bool(false)
	}

	slog.Info("Initializing moses server")
	initResp := newExtServerResp(128)
	defer freeExtServerResp(initResp)
	C.dyn_moses_server_init(llm.s, &sparams, &initResp)
	if initResp.id < 0 {
		mutex.Unlock()
	...
	slog.Info("Starting moses main loop")
	C.dyn_moses_server_start(llm.s)
	return llm, nil
}
...
func (llm *dynExtServer) Predict(ctx context.Context, predict PredictOpts, fn func(PredictResult)) error {
	...
	for retries := 0; retries < maxRetries; retries++ {
		...
		C.dyn_moses_server_completion(llm.s, req, &resp)
		if resp.id < 0 {
	...
	out:
		for {
			...
			case <-ctx.Done():
				...
				C.dyn_moses_server_completion_cancel(llm.s, resp.id, &resp)
				if resp.id < 0 {
					return extServerResponseToErr(resp)
				} else {
					return nil
			...
			default:
				...
				C.dyn_moses_server_completion_next_result(llm.s, resp.id, &result)
				json_resp := C.GoString(result.json_resp)
				C.dyn_moses_server_release_task_result(llm.s, &result)
...
func (llm *dynExtServer) Encode(ctx context.Context, prompt string) ([]int, error) {
	...
	resp := newExtServerResp(128)
	defer freeExtServerResp(resp)
	C.dyn_moses_server_tokenize(llm.s, req, &json_resp, &resp)
	if resp.id < 0 {
		return nil, extServerResponseToErr(resp)
	}
	defer C.dyn_moses_server_release_json_resp(llm.s, &json_resp)
...
func (llm *dynExtServer) Decode(ctx context.Context, tokens []int) (string, error) {
	...
	resp := newExtServerResp(128)
	defer freeExtServerResp(resp)
	C.dyn_moses_server_detokenize(llm.s, req, &json_resp, &resp)
	if resp.id < 0 {
		return "", extServerResponseToErr(resp)
	}
	defer C.dyn_moses_server_release_json_resp(llm.s, &json_resp)
...
func (llm *dynExtServer) Embedding(ctx context.Context, input string) ([]float64, error) {
	...
	resp := newExtServerResp(128)
	defer freeExtServerResp(resp)
	C.dyn_moses_server_embedding(llm.s, req, &json_resp, &resp)
	if resp.id < 0 {
		return nil, extServerResponseToErr(resp)
	}
	defer C.dyn_moses_server_release_json_resp(llm.s, &json_resp)
