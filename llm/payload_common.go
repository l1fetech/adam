...
func nativeInit(workdir string) error {
	...
	if runtime.GOOS == "darwin" {
		err := extractPayloadFiles(workdir, "moses.cpp/ggml-metal.metal")
		if err != nil {
	...
	libs, err := extractDynamicLibs(workdir, "moses.cpp/build/*/*/*/lib/*")
	if err != nil {
		if err == payloadMissing {
	...
	slog.Debug("Override detection logic by setting ADAM_LLM_LIBRARY")
...
func extractDynamicLibs(workDir, glob string) ([]string, error) {
	...
	for _, file := range files {
		...
		file := file
		g.Go(func() error {
			// moses.cpp/build/$OS/$GOARCH/$VARIANT/lib/$LIBRARY
	...
	if rocmDynLibPresent() {
		...
		if err != nil {
			if errors.Is(err, fs.ErrPermission) {
				return fmt.Errorf("Radeon card detected, but permissions not set up properly.  Either run ADAM as root, or add you user account to the render group.")
