...
function git_module_setup {
    # TODO add flags to skip the init/patch logic to make it easier to mod moses.cpp code in-repo
    & git submodule init
    if ($LASTEXITCODE -ne 0) { exit($LASTEXITCODE)}
    & git submodule update --force "${script:mosescppDir}"
    if ($LASTEXITCODE -ne 0) { exit($LASTEXITCODE)}
}

function apply_patches {
    # Wire up our CMakefile
    if (!(Select-String -Path "${script:mosescppDir}/examples/server/CMakeLists.txt" -Pattern 'adam')) {
        Add-Content -Path "${script:mosescppDir}/examples/server/CMakeLists.txt" -Value 'include (../../../ext_server/CMakeLists.txt) # adam'
    }
    ...
    foreach ($patch in $patches) {
        ...
        foreach ($file in $filePaths) {
            Set-Location -Path ${script:mosescppDir}
            git checkout $file
        }
    }
    ...
    $content = Get-Content -Path "${script:mosescppDir}/examples/server/server.cpp"
    $content = $content -replace 'int main\(', 'int __main('
    Set-Content -Path "${script:mosescppDir}/examples/server/server.cpp" -Value $content
}

function build {
    write-host "generating config with: cmake -S ${script:mosescppDir} -B $script:buildDir $script:cmakeDefs"
    & cmake --version
    & cmake -S "${script:mosescppDir}" -B $script:buildDir $script:cmakeDefs
    if ($LASTEXITCODE -ne 0) { exit($LASTEXITCODE)}
...
function install {
    ...
    cp "${script:buildDir}/bin/${script:config}/moses.dll" "${script:buildDir}/lib"
...
function cleanup {
    Set-Location "${script:mosescppDir}/examples/server"
...
$script:buildDir="${script:mosescppDir}/build/windows/${script:ARCH}/cpu"
...
$script:buildDir="${script:mosescppDir}/build/windows/${script:ARCH}/cpu_avx"
...
$script:buildDir="${script:mosescppDir}/build/windows/${script:ARCH}/cpu_avx2"
...
if ($null -ne $script:CUDA_LIB_DIR) {
    ...
    init_vars
    $script:buildDir="${script:mosescppDir}/build/windows/${script:ARCH}/cuda$script:CUDA_VARIANT"
...
$script:buildDir="${script:mosescppDir}/build/windows/${script:ARCH}/rocm"
