...
if [ -z "${ADAM_SKIP_CPU_GENERATE}" ]; then
    # Users building from source can tune the exact flags we pass to cmake for configuring
    # moses.cpp, and we'll build only 1 CPU variant in that case as the default.
    if [ -n "${ADAM_CUSTOM_CPU_DEFS}" ]; then
        echo "ADAM_CUSTOM_CPU_DEFS=\"${ADAM_CUSTOM_CPU_DEFS}\""
        CMAKE_DEFS="${ADAM_CUSTOM_CPU_DEFS} -DCMAKE_POSITION_INDEPENDENT_CODE=on ${CMAKE_DEFS}"
        ...
        if [ -z "${ADAM_CPU_TARGET}" -o "${ADAM_CPU_TARGET}" = "cpu" ]; then
            #
        ...
        if [ -z "${ADAM_CPU_TARGET}" -o "${ADAM_CPU_TARGET}" = "cpu_avx" ]; then
            #
        ...
        if [ -z "${ADAM_CPU_TARGET}" -o "${ADAM_CPU_TARGET}" = "cpu_avx2" ]; then
            #
